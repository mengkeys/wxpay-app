 ...
 */
// 推送的数据校验
WeChatPay.prototype.validate = function (xml, callback) {
    const  _this = this;
    return util.parseXML(xml, function (err, json) {
        ...
        } else if(_this.cfg.appid !== data.appid) {
            error = new Error();
            error.name = 'InvalidAppId';
        } else if (_this.cfg.mch_id && _this.cfg.mch_id !== data.mch_id) {
            error = new Error();
            error.name = 'InvalidSubMchId';
        } else if (_this.getSign(data) !== data.sign) {
            error = new Error();
